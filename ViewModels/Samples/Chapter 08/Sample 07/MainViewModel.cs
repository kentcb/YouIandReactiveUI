namespace Book.ViewModels.Samples.Chapter08.Sample07
{
    using System;
    ...
    [Sample(
        "Combined Command",
        @"This sample demonstrates the use of `ReactiveCommand` to create combined commands.

`RemoveErasCommand` will remove any era names from the given body of text. It can only execute if the text contains at least one era name. Similarly, `RemovePeriodsCommand` does the same for periods. `RemoveBothCommand` is a `CombinedReactiveCommand` that executes both commands. It can only execute if both commands can also execute.
